...
public class FileService {
    String uploadPath = "C:\\Users\\tj-bu-703-05\\Desktop\\tj_2024A_spring\\src\\main\\resources\\static\\upload\\";


    // [1] 파일 업로드
        // 매개변수로 파일의 바이트가 저장돤 MultipartFile 인터페이스
        // 업로드 된 파일명 반환
    public String filedUpload(MultipartFile multipartFile){
        ...
        // 5. transferTo (file 객체) file 객체내 설정한 해당 경로로 파일 복사/저장/이동
            // 일반 예외 무조건 발생
        try {
    ...
    @Autowired private HttpServletRequest request;  // HTTP 요청 객체 , HTTP 로 요청 들어온 정보 와 기능이 포함된 객체
    @Autowired private HttpServletResponse response; // HTTP 응답 객체 , HTTP 로 등답 할때 정보 와 기능이 포함된 객체
    ...
    public void fileDownLoad( String filename){
        ...
        // [3] HTTP 스트림으로 응답하기
            // 3-1 출력스트림 객체 생성 , new BufferedInputStream ( 출력할 대상의 스트림 객체 )
                // response.getDutputStream() : HTTP 응답할때 바이트 형식의 스트림 사용
            // BufferedOutputStream fout = new BufferedOutputStream( response.getOutputStream() );
            ServletOutputStream fout = response.getOutputStream();

            // --- HTTP 응답의 헤더 속성 추가 .setHeader(key , value)
                // Content-Disposition : 브라우저가 제공하는 다운로드 방식
