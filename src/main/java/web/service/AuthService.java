...
public class AuthService {
    ...
    @Autowired HttpServletRequest request;  // HTTP 요청객체
    ...
    public boolean authCode(String email){
        try {
            ...
            // 1. 서버 세션의 객체 속성의 인증코드를 저장
            request.getSession().setAttribute("authCode", authCode);
            // 2. 서버 세션 객체의 생명주기(세션이 유지되는 시간) // 초 기준 // 10초동안 세션 유지하고 10초 후 삭제
    ...
    // 3. 이메일 전송 함수 , 매개변수 : 받는사람의 이메일 ,메일제목 , 메일내용
    public void emailSend(String toEmail , String subject , String content){
        try {
            // 1. 메일 내용물들을 포멧/형식 맞추기위해 MIME
            // 1. Mime 객체 생성
            MimeMessage mimeMessage = javaMailSender.createMimeMessage();
            // 2. 메일내용 구성
            // new MimeMessageHelper(mimeMessage(mime객체 , 첨부파일여부 , 인코딩)
            MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(mimeMessage, true, "utf-8");
            // 3. 메일 보내는 사람
            mimeMessageHelper.setFrom("tjwlsvy1@naver.com");
            // 4. 메일 받는 사람
