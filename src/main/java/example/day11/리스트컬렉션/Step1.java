...
public class Step1 {
    public static void main(String[] args) {
        ...
        // 9-3 forEach 함수 , 요소의 개수를 하나씩 반환해서 반복 return 없음
        list.forEach(s->{
            System.out.println(s);
        });
        // 9-4 .stream().map().collect() , 요소의 개수를 하나씩 반환해서 반복후 결과(return)해서 반환
            // 반복하면서 return 값들을 하나의 배열/컬렉션으로 반환 받을수 있다.
            // 주로 카피/복사 할때 사용된다. 반복하면서 실행결과를 반환 받을수 있다.
        List<String> newList = list.stream().map( s -> {return s;} ).collect(Collectors.toList());
