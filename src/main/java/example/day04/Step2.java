...
public class Step2 {
    public static void main(String[] args) {
        ...
        // 활용 : for 문과 활용하면 문자1개씩 순회
        for (int i = 0; i<ssn.length(); i++){
            System.out.println(ssn.charAt(i));  // 문자열내 i번째 인덱스의 문자 1개씩 출력
        }
        
        // [3] 문자열.replace(기존문자열 , 변경할자열); : 기존 문자열이 존재하면 새로운 문자열로 치환/교체 된 새로운 문자열 반환
        String oldStr = "자바의 문자열은 불면입니다. 자바 문자열은 String 입니다.";
        String newStr = oldStr.replace("자바", "JAVA"); // 자바 --> JAVA 치환/교체
        System.out.println("oldStr = " + oldStr);
        System.out.println("newStr = " + newStr );
        
        // 활용 : 서로다른 언어들간의 문법 치환/교체 , html 줄바꿈<br/> 자바 \n
        ...
        // [4] 문자열.subString(시작인덱스 , [끝인덱스]) : 문자열을 인덱스 기준으로 잘라낸 새로운 문자열 반환
            // ssn = 012312 - 1230123
        String firsNum = ssn.substring(0,6);    // 0번 인덱스부터 6번 인덱스 전까지 추출
        System.out.println("firsNum = " + firsNum);

        String endNum = ssn.substring(7);   // 7번 인덱스부터 끝까지(끝 인덱스)
        System.out.println("endNum = " + endNum);
        
        // [5] 문자열.split("구분문자") : 문자열내 구분문자 기준으로 분해해서 새로운 문자열 배열 반환
        ...
        System.out.println("[0] :" + strArray[1]);  // 1230123
        ...
        for (int i =0; i < rowsStr.length; i++ ){
            ...
            for (int j = 0; j < colsStr.length; j++){   System.out.println(colsStr[j]);
            }
        }

        // [6] 문자열.indexOf("찾을문자열") : 문자열내 찾을 문자가 존재하면 찾은문자열의 인덱스 반환 없으면 -1 반환
        String subject = "자바 프로그래밍 언어";

        int findIndex = subject.indexOf("자바"); // 찾은 문자열의 인덱스 번호 반환
        System.out.println("findIndex = " + findIndex); // 자바 : 0 , 프로 : 3 , 파이선 : -1  


        // [7] 문자열.contains("찾을문자열") : 문자열내 찾을 문자가 존재하면 true 반환 , 없으면 false 반환
        boolean findCheck = subject.contains("자바");
        System.out.println("findCheck = " + findCheck); // 자바 : true , 프로 : true , 파이썬 : false
        
        // [8] 문자열.getBytes() : 문자열내 문자 하나씩 바이트로 반환된 바이트 배열로 반환
        byte[] bytes = subject.getBytes();
        System.out.println("Arrays.toString(bytes) = " + Arrays.toString(bytes));

        // 문자와 바이트 관계
            // 1. 영문/특정 특수문자 : 문자 1개당 1바이트
            // 2. 한글 : 문자1개당 2바이트 + 규칙(UTF-8 인코딩헤더규칙) 1바이트 => 총 3바이트
        byte _byte1 = 'a';
        System.out.println("_byte1 = " + _byte1);       // a , 97 // 유니코드(아스키코드)
        _byte1++;
        ...
        System.out.println("_char = "+(int)_char1);     // 44032

        char _char2 = 'a';
        System.out.println("_char2= "+(int)_char2 );    // 97
        // [활용] 난수 문자열을 만들기 ,
        System.out.println(new Random().nextInt()); // int 타입의 허용범위내 난수생성
        System.out.println(new Random().nextInt(26));   // 0 ~ 25 사이의 난수 생성
        System.out.println(new Random().nextInt(26)+97); // 97 ~ 122 사의 난수 생성
        System.out.println((char)(new Random().nextInt(26)+97)); // a ~ z 사의 난수 생성
        String newPwd = "";
        for ( int i = 0; i < 10; i++){
