...
function phoneCheck(){
    ...
    // 2. 정규표현식 : 000-0000-000 또는 00-000-0000
    ...
    if (phoneReg.test(phone)){
        ...
        phoneCheckBox.innerHTML = '000-0000-0000 또는 00-000-0000 형식으로 입력하세요'
        checkArray[3] = false;
    }
}
...
function emailCheck(){
    ...
    if (emailReg.test(email)){
        ...
        // 이메일 인증검사
            // 1. 인증버튼 활성화
            authBtn.disabled = false;   // 인증요청버튼 활성화
    }else{emailCheckBox.innerHTML='id@도메인주소 형식으로 입력해주세요.'
    }
}
// 이메일 인증 버튼
...
// 7. 인증 요청 함수
 ...
 // ---- AJAX 인증
    ...
    authBtn.disabled = true;    // 인증요청버튼 비활성화
     // 1. 인증 번호 입력 구역 구성
 ...
 function doAuthCode(){
    ...
    // * 임의의 인증번호(JS에서 인증번호를 관리하지 않는이유 : JS는 클라이언트로부터 오픈코드 이기 때문에)
    $.ajax({
        async : false ,
        method : "post" ,
        url : "/auth/check" ,
